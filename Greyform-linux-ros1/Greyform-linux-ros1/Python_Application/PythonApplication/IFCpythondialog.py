...
class ProgressBarDialogIFC(QDialog):
    ...
    def ifcprogramexecute(self):
        ...
        try:
            ...
            origin_file, origin_line, origin_func, origin_code = tb[-1]  # deepest call
            full_trace = traceback.format_exc()
            ifcmaterials.log_error(
                f"Failed to initialize IFC geometry settings or iterator:\n"
                f"{str(e)}\n"
                f"Originated from File \"{origin_file}\", Line {origin_line}, in {origin_func}\n"
                f"Code: {origin_code.strip()}\n"
                f"Full Traceback:\n{full_trace}"
            )
        self.close()
