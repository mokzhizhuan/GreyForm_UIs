...
def switchHiddenCamera(wall_name , ren , walls,renderwindowinteractor):
    """
    Removes the previous camera and adds a new hidden camera facing a specific wall.
    ...
    # Remove the previous camera by resetting to default
    if hasattr("hidden_camera") and hidden_camera:
        ren.RemoveAllViewProps()  # Removes the current camera view props
        ren.SetActiveCamera(vtk.vtkCamera())  # Reset camera
        print(f"Previous camera removed.")

    # Get wall position and normal direction
    wall_data = walls[wall_name]
    wall_position = wall_data["position"]

    # Determine an appropriate distance to place the camera
    offset = 300  # Adjust distance from the wall

    # Compute camera position based on wall orientation
    rotation = wall_data["rotation"]
    if rotation == (90, 0, 90):  # Wall facing along X-axis
        camera_position = (wall_position[0] - offset, wall_position[1], wall_position[2] + 50)
    elif rotation == (0, 90, 90):  # Wall facing along Y-axis
        camera_position = (wall_position[0], wall_position[1] - offset, wall_position[2] + 50)
    else:  # Default fallback
        camera_position = (wall_position[0], wall_position[1], wall_position[2] + offset)
    ...
    ren.SetActiveCamera(hidden_camera)
    ren.ResetCameraClippingRange()
    renderwindowinteractor.GetRenderWindow().Render()

    print(f"Switched camera to face {wall_name} at {camera_position}")
