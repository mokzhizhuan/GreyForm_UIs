...
def is_master_up(host: str = "127.0.0.1", port: int = ROS_PORT, timeout: float = 0.1) -> bool:
...
def _wait_port_up(host: str = "127.0.0.1", port: int = ROS_PORT, timeout: float = 10.0) -> bool:
...
def start_roscore(log: bool = True) -> None:
    ...
    if shutil.which("roscore") is None:
        raise RuntimeError("roscore not found on PATH. Did you `source /opt/ros/noetic/setup.bash`?")
