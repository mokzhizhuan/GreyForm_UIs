...
def initialize_walls(wallformat, walls):
    color_map = [
        (0, 1, 1),
        (1, 0, 0),
        (1, 0, 1),
    ...
    for i, wall_dict in enumerate(wallformat):  # i will be used as numeric color index
        for wall_id, wall_data in wall_dict.items():
            ...
            pos_x_range = wall_data.get(
                "pos_x_range", [wall_data["x"], wall_data["x"] + width]
            )
            pos_y_range = wall_data.get(
                "pos_y_range", [wall_data["y"], wall_data["y"] + width]
            )
